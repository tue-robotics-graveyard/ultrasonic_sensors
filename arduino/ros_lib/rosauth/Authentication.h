...
static const char AUTHENTICATION[] = "rosauth/Authentication";
  ...
  class AuthenticationRequest : public ros::Msg
  {
    ...
    {
      int offset = 0;
      uint32_t length_mac = strlen( (const char*) this->mac);
      memcpy(outbuffer + offset, &length_mac, sizeof(uint32_t));
      offset += 4;
      memcpy(outbuffer + offset, this->mac, length_mac);
      offset += length_mac;
      uint32_t length_client = strlen( (const char*) this->client);
      memcpy(outbuffer + offset, &length_client, sizeof(uint32_t));
      offset += 4;
      memcpy(outbuffer + offset, this->client, length_client);
      offset += length_client;
      uint32_t length_dest = strlen( (const char*) this->dest);
      memcpy(outbuffer + offset, &length_dest, sizeof(uint32_t));
      offset += 4;
      memcpy(outbuffer + offset, this->dest, length_dest);
      offset += length_dest;
      uint32_t length_rand = strlen( (const char*) this->rand);
      memcpy(outbuffer + offset, &length_rand, sizeof(uint32_t));
      offset += 4;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      int offset = 0;
      uint32_t length_mac;
      memcpy(&length_mac, (inbuffer + offset), sizeof(uint32_t));
      offset += 4;
      ...
      memcpy(&length_client, (inbuffer + offset), sizeof(uint32_t));
      offset += 4;
      ...
      memcpy(&length_dest, (inbuffer + offset), sizeof(uint32_t));
      offset += 4;
      ...
      memcpy(&length_rand, (inbuffer + offset), sizeof(uint32_t));
      offset += 4;
